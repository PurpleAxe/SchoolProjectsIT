...
type
  TC4 = class(TForm)
    ...
    procedure btnP4Click(Sender: TObject);
...
var
  ...
  p1XY,P2XY,P3XY,P4XY, UC, c, rand: integer;
...
begin
    ...
    if (pName = 'P3') and (OWNP3.bal > 0) then
      begin
        UC := UC + 50;
        OWNP3.bal := OWNP3.bal - 50;
      end;
    if (pName = 'P4') and (OWNP4.bal > 0) then
      begin
        UC := UC + 50;
        OWNP4.bal := OWNP4.bal - 50;
      end;
end;
...
if player = 'P3' then
  begin
    OWNP3.bal := OWNP3.bal + amt;
  end;
if player = 'P4' then
  begin
    OWNP4.bal := OWNP4.bal + amt;
  end;
end;
...
if Name = 'P3' then
  begin
    P3STATS.PMemo.Lines.Add(Place);
    properties[Index] := 'P3';
  end;
if Name = 'P4' then
  begin
    P4STATS.PMemo.Lines.Add(Place);
    properties[Index] := 'P4';
  end;
end;
  ...
  begin
    ...
    if rand = 1 then
      begin
        ...
        if (pName = 'P3')  then
          begin
            OWNP3.bal := OWNP3.bal +UC;
            UC := UC - UC;
          end;
        if (pName = 'P4')  then
          begin
            OWNP4.bal := OWNP4.bal +UC;
            UC := UC - UC;
          end;
      end;
    if rand = 2 then
...
procedure TC4.btnP1Click(Sender: TObject);
begin
P1STATS.show;
end;

procedure TC4.btnP2Click(Sender: TObject);
begin
P2STATS.show;
end;

procedure TC4.btnP3Click(Sender: TObject);
begin
P3STATS.show;
end;

procedure TC4.btnP4Click(Sender: TObject);
begin
P4STATS.show;

end;

procedure TC4.btnRDClick(Sender: TObject);
var
RD1: integer;
d: boolean;
begin
...
begin
  ...
  p3.Top := p1Ypos[p3XY];
  placement(p3XY, P3);
  label2.Caption := getUser('P3');
  d := false;
  c := 3;
end;
if (c = 3) and (d = true) then
begin
  p4XY := P4XY + RD1;
  if p4XY > 40 then
    begin
...
procedure TC4.Timer1Timer(Sender: TObject);
begin
label1.Caption := 'R '+inttostr(UC);
smP1.Lines.Strings[2] :='R ' + inttostr(OWNP1.bal);
smP2.Lines.Strings[2] :='R ' + inttostr(OWNP2.bal);
smP3.Lines.Strings[2] :='R ' + inttostr(OWNP3.bal);
smP4.Lines.Strings[2] :='R ' + inttostr(OWNP4.bal);
end;
